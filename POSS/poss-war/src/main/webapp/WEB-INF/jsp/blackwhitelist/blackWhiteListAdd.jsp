...
function checkIsExsit() {
	...
	var listType=$("#listType").val();
	var url="${ctx}/blackWhiteListAddSave.do";
	if(listType==1){
		url="${ctx}/whiteListAddSave.do";
	}
	url+="?method=checkIsExsit&listType"+listType;
	$.ajax({
		type: "POST",
		url:url,
		data: $("form").serialize(),
...
<form id="businessTypeForm" name="businessTypeForm" method="post" <c:if test="${listType==2}">action="blackWhiteListAddSave.do"</c:if>
<c:if test="${listType==1}">action="whiteListAddSave.do"</c:if>>
	...
	<table class="border_all2" width="700" border="1" cellspacing="0" cellpadding="0" align="center">
		...
		<tr class="trForContent1">
			...
			<td class="border_top_right4"  align="left">
				...
				<input type="hidden" id="approvalType" name="approvalType" value="1"/>
			</td>
		</tr>
