...
<script type="text/javascript">
		...
		$.ajax({
			...
			success : function(result) {
				$.each(result,function(i,item){
					if(item.id==layerUser){
						$("#superior").append("<option value="+item.id+" selected=selected>"+item.name+"</option>");	
					}else{
	...
	$(document).ready(function(){
		...
		//不是BD
		<c:if test="${userRelation == null}">
			$("#ifBD option:eq(1)").removeAttr("selected");
			$("#ifLeader option:eq(1)").removeAttr("selected");
			$("#layerUser").removeAttr("value");
		</c:if> 
				//判断是不是 BD
			<c:if test="${userRelation != null}">
				$("#ifBD option:eq(1)").attr("selected","selected");
				//判断是不是leader
			<c:if test="${userRelation.layer == null}">
				$("#ifLeader option:eq(1)").attr("selected","selected");	
			</c:if>
			</c:if> 
	});
	
	function sFuntion(){
		$.each($("#superior option"),function(i){
...
<table class="border_all2" width="500" border="0" cellspacing="0" cellpadding="3" align="center">
	...
	<tr class="trForContent1">
		<td align="right" class="border_top_right4" >手机</td>
		<td class="border_top_right4" >
	...
	<tr class="trForContent1">
		<td align="right" class="border_top_right4" >是否市场部leader</td>
		<td class="border_top_right4" >
	...
	<tr class="trForContent1">
		<td align="right" class="border_top_right4" >上级领导</td>
			<td class="border_top_right4" >
			<select id="superior" style="width:132px; height:20px;" onclick="sFuntion();">
	      			<option value="" ></option>
	    	</select>
	...
	<tr class="trForContent1" align="center">
		<td class="border_top_right4" align="center" colspan="4">
		 	<input type="button" class="button2" name="Submit" onclick="isBD();" value="提交">
