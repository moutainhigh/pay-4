...
<head>
	<script type="text/javascript">
		...
		function processEdit(id){
			location.href = "${ctx}/blacklist.do?method=updateBlacklist2PInit&hmdid=" + id;
		}
		function processView(id){
			location.href = "${ctx}/blacklist.do?method=blacklist2PDetail&hmdid=" + id;
		}
		function processDelete(id,name){
			var flag = confirm("确认删除姓名为: "+name+" 的个人名单吗？");
			if(flag){
		...
		function processUpload(id,name){
			var flag = confirm("确认上传姓名为: "+name+" 的个人名单吗？");
			if(flag){
				$('#infoLoadingDiv').dialog('open');
				$.ajax({
					type: "POST",
					url: "${ctx}/blacklist.do?method=uploadBlacklist2P&hmdid=" + id,
					data: "",
					success: function(result) {
						$('#infoLoadingDiv').dialog('close');
						query();
		...
		function batchUpload(key,obj){
			...
			$.ajax({
				type: "POST",
				url: "${ctx}/blacklist.do?method=batchUploadBlacklist2P",
				data: pars,
				success: function(result) {
					$('#infoLoadingDiv').dialog('close');
					query();
		...
		function batchAudit(status){
			...
			$('#infoLoadingDiv').dialog('open');
			var pars = $("#batchForm").serialize() + "&status=" + status ;
			$.ajax({
				type: "POST",
			...
			<tr>
				...
				<td>
					...
					<c:if test="${dto.status==2 || dto.status==3 || dto.status==4 || dto.status==5}">
						<a href="${ctx}/blacklist.do?method=auditInit&hmdid=${dto.hmdid}"  class="nyroModal">审核</a>
					</c:if>
