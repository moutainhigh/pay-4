...
<h2 class="panel_title">出款失败录入</h2>
<h3><font color="red">${error}</font></h3>
<form  method="post" name="mainfrom" id="mainfrom" >
	 <table class="border_all2" width="80%" border="0" cellspacing="0"
		cellpadding="1" align="center">	
	    <tr class="trForContent1">
	      	<td align="center" class="border_top_right4">商户订单号：
	      	 <input	type="text" onkeyup="checkNum(this);" name="orderId">
			</td>
	    ...
	    <tr class="trForContent1">
	      	<td align="center" class="border_top_right4">创建时间：
      			...
      			<td class="border_top_right4" colspan="4">出款状态：
				<select name="status" id='status'  > 
						<option value="">--请选择--</option>
						<option value="0"  <c:if test="${status == '0'}">selected</c:if> >待出款</option>
						<option value="1" <c:if test="${status == '1'}">selected</c:if> >出款中</option>
						<option value="3" <c:if test="${status == '3'}">selected</c:if> >出款成功</option>
						<option value="4" <c:if test="${status == '4'}">selected</c:if> >出款失败</option>
				</select>
      		</td>
      		</tr>
      		<tr class="trForContent1">
