		...
		<c:forEach items="${result}" var="dto" varStatus="status">
	       ...
	       <c:otherwise>
			...
			<input type="checkbox" name="refundInfos" 
					value="${dto.orderId},${dto.status},${dto.cpType},${dto.merchantCode}"/>
			</td>		
				<td>${dto.batchNo}</td>
				<td>${dto.orderId}</td>
				<td>${dto.partnerId}</td>
				...
				<td>${dto.visableCode}</td>			
				<td>${dto.remark}</td>			
				<td>
					<input id="detail" type="button" onclick="queryDetail('${dto.orderId}');"
					value="查 看" class="button2" />
				</td>
			</tr>
		</c:forEach>
	</tbody>
...
<br/>
<center>
   <input type="button" onclick="handerAuditBatch(1,this);" class="button2" value="复核通过"/>
   <input type="button" onclick="handerAuditBatch(0,this);" class="button2" value="复核驳回"/>
 </center>
</body>
...
function handerAuditBatch(status,obj,pageNo,totalCount,pageSize){
	...
	if(confirm("您是否确定【" + obj.value + "】提交?")){
		...
		$.ajax({
			type: "POST",
			url: "bounced-register.do?method=handerAuditBatchApp",
			data: pars,
			success: function(result) {
				alert("处理成功！");
				$('#infoLoadingDiv').dialog('close');
...
function queryDetail(orderId){
	...
	$.ajax({
		type: "POST",
		url: "bounced-register.do?method=queryApproveDetail",
		data: pars,
		success: function(result) {
			$('#infoLoadingDiv').dialog('close');
