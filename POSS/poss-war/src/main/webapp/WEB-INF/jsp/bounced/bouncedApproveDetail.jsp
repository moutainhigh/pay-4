...
<form action="refund.manage.do?method=handerRefundAuditSingle" method="post" name="auditForm">
	...
	<br/>
	<div align="left" style="width: 80%">
		<font style="text-align: left;" color="blue"><b>操作记录</b></font>
		...
		操作备注：<textarea rows="5" cols="60" name="remark" 
							id="remark" title="最多只能输入50个汉字或100个字符"></textarea>
	</div>
	<br/>
	<p/>
	<center>
   	 <input type="button" onclick="handerAuditBatch(1,'${mDto.status}','${mDto.orderId}','${mDto.cpType}','${mDto.merchantCode}');" class="button4" value="复核通过"/>
     <input type="button" onclick="handerAuditBatch(0,'${mDto.status}','${mDto.orderId}','${mDto.cpType}','${mDto.merchantCode}');" class="button4" value="复核驳回"/>
    <input type="button" name="btnRollback" onclick="rollbackAuditQuery();" class="button4" value="返  回"/>
    </center>
</form>
 <script type="text/javascript">

 function handerAuditBatch(status,statusBegin,orderId,cpType,merchantCode){
		var flag = true;
		if(1 == status){
			if('3' != statusBegin && '4' != statusBegin){
				alert("银行未回复!");
				flag = false;
				return false;
			}else if('3' == statusBegin)
			{
				status=5
			}else if('4' == statusBegin)
			{
				status=6
			}
		}
		...
		if(confirm("您是否确定提交?")){
			...
			var pars = "&status=" + status  
						+ "&orderId=" + orderId
						+ "&gcFlag=" + merchantCode
						+"&remark=" + remark+"&bouncedType="+cpType;
			$.ajax({
				type: "POST",
				url: "bounced-register.do?method=auditBatchApp",
				data: pars,
	...
	function rollbackAuditQuery(){
		var url = 'bounced-register.do?method=bouncedApprove';
