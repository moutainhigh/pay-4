...
<head>
	...
	//id的全选或全部取消
		...
		function showDetail(detailUrl,workOrderKy){
			parent.addTabMenu('查看分控已处理数据详细',detailUrl,workOrderKy);
		}
	</script>
</head>
	...
	<table id="userTable" class="tablesorter" border="0" cellpadding="0" cellspacing="1">
		...
		<c:choose>
	       ...
	       <c:otherwise>
				...
				<td>${orderDto.auditUser}</td>
				<td>${orderDto.reAuditUser}</td>
				<td>${orderDto.statusStr}</td>				
				<td><a href="javascript:showDetail('${ctx}/withdrawreport.htm?method=showDisposedDetailInfo&orderId=${orderDto.sequenceId}&workOrderKy=${orderDto.workOrderky}','${orderDto.sequenceId}')">查看</a></td> 
			</tr>
			</c:forEach>
		</tbody> 
	</table>
