...
<h2 class="panel_title">商户交易汇率微调导入</h2>
<form action="${ctx}/transactionRateAdjust/adjust.do?method=add" method="post" id="mainfromId" enctype="multipart/form-data" onsubmit="return sub();">
...
<c:if test="${not empty responseMsg}">
  ...
  function download(){
		window.location.href='${ctx}/transactionRateAdjust/adjust.do?method=downloadTemplate';
	}
  
	function toIndex(){
		window.location.href="${ctx}/transactionRateAdjust/adjust.do";
	}
