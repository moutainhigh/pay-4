...
<form name="assignTaskForm" method="post" action="refund.manage.do?method=handerAssignTask">
...
<br/>
<center>
			选择操作员:<select id="assigner" name="assigner" style="width:120px;"></select>
<br/>
<br/>    
	      <input type="button" onclick="assignTask();" class="button4" value="任务分配"/>
	   </center>

<script type="text/javascript">
	var queryUserFlag = false;
	
	function assignTask(pageNo,totalCount,pageSize){
		...
		$("input[name='refundInfos']:checked").each(function(index){
			...
			if(0 == index){
				...
				if(tempCh != initStatus){
					if(initStatus == array[2]){
		...
		if(confirm("您是否确定【任务分配】提交?")){			
			...
			$.ajax({
				type: "POST",
				url: "refund.manage.do?method=handerAssignTask",
				data: pars,
