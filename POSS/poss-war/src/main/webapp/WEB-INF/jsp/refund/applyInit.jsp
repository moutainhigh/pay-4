  ...
  <table class="border_all2" width="80%" border="0" cellspacing="0"
	...
	<tr class="trForContent1">
      	...
      	<td class="border_top_right4">
	      	<input class="Wdate" type="text" id="startTime"  name="startTime" value='<fmt:formatDate value="${webQueryRefundDTO.startTime}" type="date"/>'  onClick="WdatePicker({minDate:'%y-%M-{%d-30}',maxDate:'#F{$dp.$D(\'endTime\')}'})">
	        	～
    ...
    <tr class="trForContent1">
      	<td class=border_top_right4 align="right" >交易流水号：</td>
       	<td class="border_top_right4" >
        	<input type="text"  name="rechargeSeq" value="${webQueryRefundDTO.rechargeSeq }"  />
     	</td>
      	<td class=border_top_right4 align="right" >银行流水号：</td>
       	<td class="border_top_right4" >
        	<input type="text"  name="bankSeq" value="${webQueryRefundDTO.bankSeq }"  />
     	</td>
    </tr>
    <tr class="trForContent1">
...
<script type="text/javascript">
	...
	function submitByHref(pageNo,totalCount,pageSize) {
		...
		var pars = $("#mainfromId").serialize() + "&pageNo=" + pageNo + "&totalCount=" + totalCount+"&pageSize="+pageSize;
			$.ajax({
				type: "POST",
				url: "refund.manage.do?method=queryReChargeList",
				data: pars,
