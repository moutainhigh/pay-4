...
<script type="text/javascript" language="javascript">
	...
	function confirmFail(){
		...
		$.ajax({
			type: "POST",
			url: "${ctx}/bankcorporateexpress.htm?method=confirmFail",
			data: pars,
 ...
 <div class="e_cur_tit2"><a href="#" onclick="exportExcel();">下载</a><span></span></div>
<table id="userTable" class="tablesorter" border="0" cellpadding="0" cellspacing="1">
	<thead> 
		<tr >
			<c:if test="${bceStatus=='14'}">
	...
	<tbody> 
		...
		<tr align="left">
			<td colspan="15">交易总金额<span id="chooseAmounts"><fmt:formatNumber value="${countAmount/1000 }" pattern="#,##0.00"  /></span>元,共<span id="chooseCounts">${pageSize }</span>笔</td>
		</tr>
	<c:if test="${page.totalCount > 0 && bceStatus=='14'}">
		<tr align="left">
			<td colspan="15">选中金额<span id="chooseAmount">0</span>元,共<span id="chooseCount">0</span>笔</td>
		</tr>
		<tr align="center">
			<td colspan="15">
				<input id="download" type="button" onclick="confirmFail();" value="确认失败" class="button2" />
				<input id="download" type="button" onclick="nextStep();" value="生成批次" class="button2" />
			</td>
		</tr>
	</c:if>
</table>
