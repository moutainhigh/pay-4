...
function indexQuery(pageNo,totalCount,pageSize) {
	...
	$.ajax({
		type: "POST",
		url: "${ctx}/productsManager.do?method=searchSpEnumInfo",
		data: pars,
	...
	$('#addInfoDiv').dialog( 
			...
			'确定':function(){ 
				...
				if(checkEnumType()){
					...
					$.post("${ctx}/productsManager.do?method=addSpEnumInfo",{enumName:$("#enumNameA").val(),enumCode:$("#enumCodeA").val(),value1:$("#value1A").val(),value2:$("#value2A").val()}
						,function (msg){
								if(msg=="S"){
									indexQuery();
	...
	$('#updateInfoDiv').dialog( 
			...
			'确定':function(){ 
				...
				if(checkEnumTypeB()){
					...
					$.post("${ctx}/productsManager.do?method=updateSpEnumInfo",{id:enumId,enumName:$("#enumNameB").val(),enumCode:$("#enumCodeB").val(),value1:$("#value1B").val(),value2:$("#value2B").val()}
						,function (msg){
								if(msg=="S"){
									indexQuery();
...
function showSpEnumInfoDetail(id){
	$('#detailDiv').load("${ctx}/productsManager.do?method=getSpEnumInfoDetail",{id:id},function(msg){
		});	
		$('#detailDiv').dialog( 
				{ 
				width:500,	
...
function delSpEnumInfo(enumId,enumName,enumCode,value1,value2){
	...
	$('#delInfoDiv').dialog( 
			...
			buttons:{ 
			'确定':function(){ 	
					$.post("${ctx}/productsManager.do?method=delSpEnumInfo",{id:$("#enumId").val()}
						,function (msg){
								if(msg=="S"){
									indexQuery();
...
<table width="35%" border="0" cellspacing="0" cellpadding="0"
	...
	<tr>
		<td height="18">
		<div align="center"><font class="titletext">经 营 范 围 维 护</font></div>
		</td>
	</tr>
	<tr>
		<td height="1" bgcolor="#000000"></td>
		...
		<table class="searchTermTable" class="inputTable" style="width:80%" >
			...
			<tr>
					...
					<a href="javascript:void(0)" onclick="addSpEnumInfo()" class="dialog_link ui-state-default ui-corner-all">
						<span class="ui-icon ui-icon-newwin"></span>&nbsp;添加经营范围&nbsp;
					</a>
				</td>
			</tr>
		</table>
...
<div id="delInfoDiv" style="display: none">
	...
	<tr>
		<th >类型</th>
		<td >
			<div id="enumTypeDiv" style="width:100%;word-break : break-all" >经营范围</div>
		</td>
	</tr>
	<tr>
		<th >备用一</th>
