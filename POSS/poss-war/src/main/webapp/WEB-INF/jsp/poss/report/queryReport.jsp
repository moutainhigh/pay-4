...
<script type="text/javascript">
		...
		function userQuery(pageNo,totalCount,pageSize){
			...
			$.ajax({
				type: "POST",
				url: "${ctx}/report/queryReport.do?method=query",
				data: pars,
		...
		function downLoad(pageNo,totalCount,pageSize){
			...
			window.location="${ctx}/report/queryReport.do?method=downloadRecordList&"+pars; 
        }
...
<form action="" method="post" id="form1" name="form1">
  <table class="border_all2" width="80%" border="0" cellspacing="0"
    ...
    <tr class="trForContent1">
    	...
    	<td class="border_top_right4">
	    		<input type="text" id="memberCode" name="memberCode" maxlength="11" size="16"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	    		<label for="r1">
					用户类型：
				</label>
    	...
    	<td align="right" class="border_top_right4">
      	...
      	<td class="border_top_right4" >
        	<select id="imemberCode" name="imemberCode" disabled="disabled">
       			<option selected="selected" value="">全部</option>
...
<script type="text/javascript">
	...
	select2[5][0] = new Option("选择所有", "");
	select2[5][1] = new Option("处理中", "0");     
	select2[5][2] = new Option("成功", "1");     
	select2[5][3] = new Option("失败", "2");    
	...
	function selectOptType(optType){	
		if(1== optType){
			...
			document.getElementById('imemberCode').disabled=true;
		}else{
			document.getElementById('memberCode').disabled=true;
			document.getElementById('memberType').disabled=true;
			document.getElementById('imemberCode').disabled=false;
		}
	}
