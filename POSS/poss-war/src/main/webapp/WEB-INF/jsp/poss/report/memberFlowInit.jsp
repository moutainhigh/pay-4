...
<script type="text/javascript">
		...
		function userQuery(pageNo,totalCount,pageSize){
			...
			$.ajax({
				type: "POST",
				url: "${ctx}/report/memberFlow.do?method=queryMemberFlow",
				data: pars,
		...
		function exportExcel(totalCount) {
			if(totalCount <= 0){
				...
				window.location="${ctx}/report/memberFlow.do?method=queryMemberFlow&export=1&"+pars; 
			}
        }
...
<h2 class="panel_title">会员日交易查询统计</h2>
...
<form action="" method="post" id="form1" name="form1">
  <table class="border_all2" width="80%" border="0" cellspacing="0"
    ...
    <tr class="trForContent1">
    	...
    	<td class="border_top_right4" >
        	<label>
				<input type="radio" id="r1"  name="optType" checked="checked" value="1" onclick="selectOptType(1)" /></label>
		  	<label for="r1">
				日交易明细统计
		    </label>
		    <label>
				<input type="radio" id="r2"  name="optType" value="2" onclick="selectOptType(2)" /></label>
		    <label for="r2">
				日交易汇总统计
			</label>
        </td>
    ...
    <tr class="trForContent1">
		...
		<td class="border_top_right4" align="left" >
				...
				<option value="" selected>信用卡</option>
			</select>
		</td>
