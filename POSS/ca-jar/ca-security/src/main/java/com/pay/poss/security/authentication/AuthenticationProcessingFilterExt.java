...
import com.pay.poss.security.service.ILoginService;
import com.pay.poss.security.service.IUserService;
import com.pay.poss.security.util.IpUtils;
import com.pay.poss.security.util.SecutiryRequestUtil;
...
public class AuthenticationProcessingFilterExt extends AuthenticationProcessingFilter{
	...
	public void doFilterHttp(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(null != randSession && null != randRequest && !"".equals(randRequest) && !"".equals(randSession)){
			if(randRequest.length() != 4){
				if(!randRequest.equals(this.PER_CODE)){
					codeV(request, response);
					return;
				}
			}else{
				if(!randRequest.equalsIgnoreCase(randSession)){
					codeV(request, response);
					return;
				}
			}
	 ...
	 * @throws IOException
	 */
	private void codeV(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		logger.warn("登录验证码有误:"+request.getRemoteHost());
		sendRedirect(request, response, SecurityConstants.ACEGI_EXCEPTION_MAPPING_ERROR_RAND_CODE);
		return;
