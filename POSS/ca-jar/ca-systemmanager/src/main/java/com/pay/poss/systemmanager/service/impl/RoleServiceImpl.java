...
import com.pay.poss.systemmanager.dao.impl.RoleDaoImpl;
import com.pay.poss.systemmanager.dao.impl.UserDaoImpl;
...
import com.pay.poss.systemmanager.service.IMenuFactoryService;
import com.pay.poss.systemmanager.service.IRoleService;
...
public class RoleServiceImpl implements IRoleService {
	...
	public boolean insertBatch(String roleKy, RoleFormBean roleFormBean,
		...
		if (StringUtils.isEmpty(roleKy)) {
			...
			Role role = (Role) daoService.findObjectByCriteria(
					"role.queryRoleByRoleKy", roleKy);
			role.setRoleName(roleFormBean.getRoleName());
			role.setRoleRemarks(roleFormBean.getRoleRemarks());
		...
		if (hasDelIds != null && hasDelIds.size() > 0) {
			...
			daoService.batchCreate("role.deleteRoleByRoleAndRes", delList);
		}
		...
		if (hasAddIds != null && hasAddIds.size() > 0) {
			...
			daoService.batchCreate("role.insertMenuOfRole", rs);
		}
	...
	public boolean deleteRole(String roleKy) throws Exception {
		List<RoleRes> roleResList = roleDao.queryRoleResByRoleKy(new Long(
				roleKy));
		daoService.deleteBatch("role.deleteRoleRes", roleResList);
		return roleDao.deleteRole(roleKy) == 1;
	}

	public Role queryRoleByRoleKy(String roleKy) {
		return (Role) daoService.findObjectByCriteria("role.queryRoleByRoleKy",
				roleKy);
	}
	...
	public boolean insertRoleOfUser(UserRole userRole) {
		...
		try {
			...
			log.error("RoleServiceImpl.insertRoleOfUser error...");
			sign = false;
		}
	...
	public boolean dropMenuOfRole(Map<String, String> map) {
		...
		try {
			...
			log.error("RoleServiceImpl.dropMenuOfRole error...");
			sign = false;
		}
	...
	public boolean dropRoleOfUser(Map<String, String> map) {
		...
		try {
			...
			log.error("RoleServiceImpl.dropRoleOfUser error...");
			sign = false;
		}
	...
	public List<UserInfo> findAllUser() {
		...
		try {
			...
			log.error("RoleServiceImpl.findAllUser error...");
		}
	...
	public List<Role> queryRoleOfUser(String userId) {
		...
		try {
			...
			log.error("RoleServiceImpl.queryRoleOfUser error...");
		}
