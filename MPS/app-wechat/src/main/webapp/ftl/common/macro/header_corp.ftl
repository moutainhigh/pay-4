			...
			<#list  selfPmList as pm>
			     ...
			     <li name="f_header" index="${rootIndex}">
			        <a name="f_header_a" id="subLi_${rootIndex}"   
			            <#if pm.menuUrl?exists && pm.menuUrl!='#'>
			                   href="<@contextPath/>${pm.menuUrl}"
			            <#else>href="javascript:void(0);"</#if>>
			            <strong>${pm.menuName}</strong>
			            <i></i></a>
						...
						<#list pm.childlist as ch>
							...
							<#if childIndex==1>
							    <script>
							           $(function(){$("#subLi_${rootIndex}").attr("href","<@contextPath/>${ch.menuUrl}");})
							    </script>
							</#if>
							<#if sp.hasMenu(ch.menuCode?default(""),mCode)>
							</#if>
								<a href="javascript:void(0);" class="" ><span  name="enabledMenu"   url="<@contextPath/>${ch.menuUrl}">${ch.menuName}</span></a>
							</#if>
							<#else>
