...
public class OrderQueryServiceImpl implements OrderQueryService {
	...
	public OrderQueryResultDTO queryTradeUnionPayment(
		...
		if (orderQueryResultDetailList != null
			...
			for (int i = 0; i < orderQueryResultDetailList.size(); i++) {
				OrderQueryResultDetail orderQueryResultDetail = orderQueryResultDetailList
						.get(i);
	...
	public OrderQueryResultDTO queryTradeUnionRefund(
		...
		refundOrderQueryResultDetailList = gatewayOrderQueryService.queryTradeUnionRefund(paramMap);
		...
		if (refundOrderQueryResultDetailList != null
			...
			for (int i = 0; i < refundOrderQueryResultDetailList.size(); i++) {
				...
				RefundOrderQueryResultDetail refundOrderQueryResultDetail = refundOrderQueryResultDetailList
						.get(i);
...
//		reconcileOrderDTO.setSettlementCurrencyCode("SettlementCurrencyCode");
	...
	public Map<String, Object> querySingleIncomeDetailForRefund(Map<String, Object> map) {
		return gatewayOrderQueryService.querySingleIncomeDetailForRefund(map) ;
	}
	
	// @Override
	@Override
	public Map<String, Object> queryRefundOrderList(Map<String, Object> map) {
		return gatewayOrderQueryService.refundOrderQuery(map);
	}
	...
	public List<QueryDetailDTO> queryIncomeDetailList(
		...
		if(queryDetailList!=null&&queryDetailList.size()>0){
			for (QueryDetail queryDetail : queryDetailList) {
				...
				if("100".equals(queryDetailDTO.getTradeType().substring(0,3))){
					queryDetailDTO.setTradeTypeDisplay("信用卡支付");
				}else if("200".equals(queryDetailDTO.getTradeType().substring(0,3))){
					queryDetailDTO.setTradeTypeDisplay("信用卡支付");
				}else if("210".equals(queryDetailDTO.getTradeType().substring(0,3))){
					queryDetailDTO.setTradeTypeDisplay("信用卡支付");
				}else if("7004,7005,7204,7205,7000,7200".contains(queryDetailDTO.getTradeType())){
					queryDetailDTO.setTradeTypeDisplay("信用卡支付");
				}else{
					String orgCode = queryDetailDTO.getOrgCode();
					if(StringUtils.isNotBlank(orgCode) && enumChannels.containsKey(orgCode)){
						String channelDesc = enumChannels.get(orgCode).getDesc();
						if(channelDesc!=null){
							queryDetailDTO.setTradeTypeDisplay(channelDesc.replace("CT_",""));
						}
					}else{
						queryDetailDTO.setTradeTypeDisplay(" ");
					}
				}
