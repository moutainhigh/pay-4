...
import com.pay.base.model.Feature;
...
public class FeatureDAOImpl extends BaseDAOImpl implements FeatureDAO  {
    ...
    public List<FeatureDto> queryAllFeature (){
        List<Feature> featureList=this.getSqlMapClientTemplate().queryForList(namespace.concat("findAllFeature"));
        List<FeatureDto> fDtoList=new ArrayList<FeatureDto>(featureList.size());
    ...
    public FeatureDto getFeatureById(Long featureId){
        Feature ft=(Feature)this.getSqlMapClientTemplate().queryForObject(namespace.concat("getByFeatureId"), featureId);
    ...
    public Long getFeatureIdByLvl(int securLevel,int appScale){
        ...
        return (Long)this.getSqlMapClientTemplate().queryForObject(namespace.concat("getLoginLevel"),map);
