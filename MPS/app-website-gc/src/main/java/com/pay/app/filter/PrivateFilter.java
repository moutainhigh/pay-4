...
public class PrivateFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest,
		...
		if (!isNeedLogin(requestPath, isLogin)
				&& (AppFilterCommon.isNormalUser(request) || AppFilterCommon
						.isCorpNormalUser(request))) {
		...
		/***************** 记录登录前请求的url ********************/
		AppFilterCommon.setCallBackUri(request, AppConf.defaultCallBack,
				OrginEnum.INDIVIDUAL_LOCAL.getValue(), 1);
		AppFilterCommon.setCallBackUri(request, AppConf.defaultCorpCallBack,
				OrginEnum.CORP_LOCAL.getValue(), 2);
		/***************** 跳登录页 ********************/
