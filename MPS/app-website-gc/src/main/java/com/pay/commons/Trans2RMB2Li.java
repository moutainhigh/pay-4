package com.pay.commons;


public class Trans2RMB2Li {
	...
	public static void main(String[] args) {
		System.out.println("\n--------将数字转换成中文金额的大写形式------------\n");
		String s = cleanZero(splitNum(roundString(getNum())));
		// 如果转换过后是一个空串，则不输出屏幕
		if (!"".equals(s)) {
	...
	public static String processNum(String num) {
	...
	private static String splitNum(String s) {
		...
		String part2 = numFormat(3, smallOnly);
	...
	private static String roundString(String s) {
		...
		d = (d * 1000 + 0.5) / 1000;
		// 将 d 进行格式化
		s = new java.text.DecimalFormat("##0.0000").format(d);
		// 以小数点为界分割这个字符串
		int index = s.indexOf(".");
		// 这个数的整数部分
		String intOnly = s.substring(0, index);
		...
		if (smallOnly.length() > 3) {
			String roundSmall = smallOnly.substring(0, 3);
