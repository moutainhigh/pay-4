  <#macro pagination pu>
	...
	<#if (pageIndex lt pageCount)>  
      	  ...
      	  <input type="hidden" id="f_pageCount" value="${pageCount}"/>
      	  <label class="totalCount">共${pageCount}页</label>
      	  <em style="position:relative;top:-5px;">
      	  	 到第<input type="text" value="${pageIndex?default('')}" id="jumpNum" onkeyup="javascript:numOnly(this);" class="toPageNum">页
      	  </em>
      	  <a href="javascript:void(0);" title="确定" onclick="javascript:jumpNumGo() ;">确定</a>
      <#else>
	 ...
	 <script type="text/javascript">
	 	function jumpNumGo(){
	 		...
	 		var pageCount = document.getElementById("f_pageCount").value ;
	 		if(Number(jumpNum) > Number(pageCount)){
	 			jumpNum = pageCount ; 
	 		}
	 		window.location.href = "${new_uri}" + jumpNum  ;
	 	}
	 	//只能输入数字
