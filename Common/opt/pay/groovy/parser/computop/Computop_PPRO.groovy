 ...
 *  @author davis.guo   
 *  @Date 2016-07-24
 *  @Description Computop 针对PPRO支付对账文件解析
 *  @Copyright Copyright © 2014-2023 ipaylinks.com . All rights reserved.
 *  请另存为*.xsl 文件，行数和 cell 都是从0 开始计数的
 */
class Computop_PPRO extends AbstractBaseFileParser {
	...
	public FileParseResult parserFile(File file) throws Exception {
		...
		String txState = "SUCCEEDED";//交易成功与否标识
		
		int rowSize = sheet.getLastRowNum();
		//交易记录处理
		for (int rowNum = 1; rowNum <= rowSize-1; rowNum++) {//从第2行开始
			Row row = sheet.getRow(rowNum);
			if(row == null)continue;
			txState = String.valueOf(row.getCell(1));
			if(!"SUCCEEDED".equalsIgnoreCase(txState))continue;
			
			totalRecord ++;
			
			def debit_credit_chargebac = String.valueOf(row.getCell(0));	//RECORD_TYPE{700:“支付” ，701:“退款”，702“拒付”) 
			def channelOrderNo = String.valueOf(row.getCell(0));	//TRANSACTION_ID渠道号
			def dealAmount = String.valueOf(row.getCell(4));		//TRANSACTION_AMOUNT交易金额
			def currency = String.valueOf(row.getCell(5));			//TRANSACTION_CURRENCY交易币种
			def dealDate = String.valueOf(row.getCell(14));			//PAYMENT_CREATION_DATE支付时间
	...
	public String changeDate(String datetime){
		...
		String date_time = datetime.replaceAll("T", " ").replaceAll("Z", "");
		/*SimpleDateFormat sdf_origin = new SimpleDateFormat("yyyy-MM-ddTHH:mm:ssZ")
