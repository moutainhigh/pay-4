 ...
 *  @author lIWEI
 *  @Date 2011-8-23
 *  @Description 中国银行 批量转账
 */
class Boc_G2E extends AbstractBankFileGenerator {
	...
	public boolean supportBank(String bankCode) {
		return "100030011".equals(bankCode);
	}
	...
	protected void buildBankDetailList(FileGenerateResult fileGenerateResult) {
		...
		for(fileDetailMode in fileGenerateResult.getDetailList()){
			...
			//金额
			def amount = BankFileUtil.divideNum(bocModel.getAmount(),new BigDecimal(1000));
			bocModel.setAmount(amount);
			//客户业务编号
			bocModel.setCustBusiSerialNo(df.format(count));
			bocModel.setExpectDate(df3.format(new Date()));
		...
		String fileName ="boc_${dateString}_${fileGenerateResult.getBatchNum()}_1";
		...
		fileGenerateResult.setTemplateName("boc_gen2e.vm");
