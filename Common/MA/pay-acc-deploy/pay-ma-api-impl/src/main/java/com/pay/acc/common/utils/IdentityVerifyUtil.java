...
public class IdentityVerifyUtil {
	...
	static byte[][] C = new byte[17][28];

	static byte[][] D = new byte[17][28];

	static byte[][] K = new byte[17][48];
	 ...
	 */
	private static void desMemcpy(byte[] output, byte[] input, int outpos, int inpos,
			int len) {
		int i;
		for (i = 0; i < len; i++)
	...
	private static void Fexpand0(byte[] in, byte[] out) {
		int divide;
		int i, j;
		byte temp1;
	...
	private static void FLS(byte[] bits, byte[] buffer, int count) {
		for (int i = 0; i < 28; i++) {
			buffer[i] = bits[(i + count) % 28];
		}
	}

	private static void Fson(byte[] cc, byte[] dd, byte[] kk) {
		int i;
	...
	private static void Fsetkeystar(byte[] bits) {
		int i, j;
	...
	private static void Fiip(byte[] text, byte[] ll, byte[] rr) {
		int i;
...
//		byte[] tmp = new byte[64];
	...
	private static void Fs_box(byte[] aa, byte[] bb) {
		int i, j, k, m;
		int y, z;
		byte[] ss = new byte[8];
		m = 0;
	...
	private static void FF(int n, byte[] ll, byte[] rr, byte[] LL, byte[] RR) {
		int i;
	...
	private static void _Fiip(byte[] text, byte[] ll, byte[] rr) {
		int i;
		byte[] tmp = new byte[64];
		for (i = 0; i < 32; i++)
			tmp[i] = ll[i];
	...
	/*private static void Fcompress016(byte[] out, byte[] in) {
		int times;
		int i, j;
	...
	static void Fcompress0(byte[] out, byte[] in) {
		int times;
		int i, j;
	...
	private static void Fencrypt0(byte[] text, byte[] mtext) {
	...
	private static void FDES(byte[] key, byte[] text, byte[] mtext) {
		byte[] tmp = new byte[64];
		Fexpand0(key, tmp);
		Fsetkeystar(tmp);
	...
	/* 加密 */
	public static int ENCRYPT(byte[] key, byte[] s, byte[] d, int len) {
		int i, j;
	...
	private static void Fdiscrypt0(byte[] mtext, byte[] text) {
		byte[] ll = new byte[64], rr = new byte[64], LL = new byte[64], RR = new byte[64];
		byte[] tmp = new byte[64];
		int i, j;
		Fiip(mtext, ll, rr);
	 ...
	 **************************************************************************/
	private static void _FDES(byte[] key, byte[] mtext, byte[] text) {
		byte[] tmp = new byte[64];
		Fexpand0(key, tmp);
		Fsetkeystar(tmp);
	...
	/* 解密 */
	public static int DECRYPT(byte[] key, byte[] s, byte[] d, int len) {
		int i;
		byte[] cData = new byte[8];
		byte[] cEncryptData = new byte[8];
		for (i = 0; i < len; i += 8) {
	 ...
	 * @return
	 */
	public static String encode(String str, String desKey) {
		int i1;
		String digestHexStr;
		byte[] byte2 = new byte[3000];
		byte[] byte1 = str.getBytes();

		i1 = ENCRYPT(desKey.getBytes(), byte1, byte2, byte1.length);
		digestHexStr = "";
		for (int i = 0; i < i1; i++) {
	...
	public static String Decode(String str) {
		...
		i2 = DECRYPT(DesUtil.strDefaultKey.getBytes(), byteMingW, byteMiW,
				byteMiW.length);

		return new String(byteMingW, 0, i2);
	}
	 */
	public String createUrl(String name,String idCode){
		String urlStr="";
		String userId="你的用户编号";
		...
		String nameDes=encode(name, desKey);
		String idCodeDes=encode(idCode, desKey);
		String md5Str = "userId" + userId + "coopOrderNo" + orderNo + "auName" + name + "auId" + idCode + "ts" + ts + mKey;
		md5Str = md5Sign(md5Str) ;
		Date currentTime = new Date();
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String dateString = formatter.format(currentTime);
		urlStr="http://192.168.11.98:1001/IdInDataAu/api/authenInfoApi.htm?"+"userId=" + userId + "&coopOrderNo=" + orderNo + "&auName=" + nameDes + "&auId=" + idCodeDes + "&reqDate=" + dateString + "&ts=" + ts + "&sign=" + md5Str;
		return urlStr;
	}
	
	public static void main(String[] args){
		System.out.println(new IdentityVerifyUtil().createUrl("哦", "32062419990108532x"));
	}
}
