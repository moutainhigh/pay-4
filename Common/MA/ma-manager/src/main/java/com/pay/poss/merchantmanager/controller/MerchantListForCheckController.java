...
public class MerchantListForCheckController extends SimpleFormController {

	private Log log = LogFactory.getLog(MerchantListForCheckController.class);

	private IMerchantService merchantService ;

	@Override
	protected Map<String, Object> referenceData(HttpServletRequest request) throws Exception {
		log.debug("MerchantListForCheckController.referenceData() is running...");
	...
	private List<MerchantSearchListDto> getNameByCode(List<MerchantSearchListDto> merchantList){
		if (merchantList != null && merchantList.size() != 0) {
			for(int i=0;i<merchantList.size();i++){
				...
				if(StringUtils.isNotEmpty(merchant.getMerchantStatus())&&StringUtils.isNotEmpty(merchant.getMerchantCheckStatus())){
					if(merchant.getMerchantStatus().equals(Constants.STATUS_UNACTIVATION.toString())&&merchant.getMerchantCheckStatus().equals(Constants.CHECK_YES.toString())){
						merchant.setIsSendEmail("yes");
					}
				}
