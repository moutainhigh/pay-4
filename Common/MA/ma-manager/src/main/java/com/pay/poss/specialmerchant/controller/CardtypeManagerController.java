...
public class CardtypeManagerController extends MultiActionController{
	...
	public ModelAndView addSpEnumInfo(HttpServletRequest request,
		...
		int maxEnumCode = spEnumInfoService.findMaxEnumCode(1);
		...
		try{
			if(page.getResult().size()==1){
				isok = false;
				exceptionMsg ="，名称重复";
			}else if("20000".equals(enumCode)){
				isok = false;
				exceptionMsg ="，代码不能超过20000";
			}else{
			...
			isok = false;
			exceptionMsg = "，后台操作异常";
			logger.error("CardtypeManagerController.addSpEnumInfo throws error", e);
			//e.printStackTrace();
		}
	...
	public ModelAndView updateSpEnumInfo(HttpServletRequest request,
		...
		try{
			...
			isok = false;
			exceptionMsg = "，后台操作异常";
			logger.error("CardtypeManagerController.updateSpEnumInfo throws error", e);
			//e.printStackTrace();
		}
