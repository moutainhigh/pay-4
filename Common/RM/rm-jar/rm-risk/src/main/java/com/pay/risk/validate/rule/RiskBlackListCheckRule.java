...
public class RiskBlackListCheckRule extends MessageRule {
	...
	private static Logger logger = LoggerFactory.getLogger(RiskBlackListCheckRule.class);
	...
	protected boolean makeDecision(Object validateBean) throws Exception {
		...
		if (StringUtil.isEmpty(ip) || !IPUtil.validateIp(ip)) {
			...
			paymentResult.setResponseDesc("IP校验不通过");
			return false;
		}
		
		// 校验IP
		blackWhiteListDtos = riskBlackListService.getBlackWhiteList("3-2");
			...
			for (BlackWhiteListDto dto : blackWhiteListDtos) {
				...
				if (dto.getPartType() == 1) {
					if (dto.getContent().equals(ip)) {
						paymentResult.setResponseCode(getMessageId());
						paymentResult.setResponseDesc("IP黑名单");
						return false;
					}
		...
		// 校验IP
		blackWhiteListDtos = riskBlackListService.getBlackWhiteList("11-2");
		if (null != blackWhiteListDtos && !blackWhiteListDtos.isEmpty()) {
			for (BlackWhiteListDto dto : blackWhiteListDtos) {
		...
		blackWhiteListDtos = riskBlackListService.getBlackWhiteList("2-2");
		if (null != blackWhiteListDtos && !blackWhiteListDtos.isEmpty()) {
			for (BlackWhiteListDto dto : blackWhiteListDtos) {
				// 账单邮箱，全匹配
				if (dto.getPartType() == 1) {
					if (dto.getContent().equalsIgnoreCase(billEmail)) {
						paymentResult.setResponseCode(getMessageId());
						paymentResult.setResponseDesc("账单邮箱黑名单");
						return false;
					}

					if (dto.getContent().equalsIgnoreCase(shippingMail)) {
						paymentResult.setResponseCode(getMessageId());
						paymentResult.setResponseDesc("收件邮箱黑名单");
						return false;
					}
					if (dto.getContent().equalsIgnoreCase(cardHolderEmail)) {
						paymentResult.setResponseCode(getMessageId());
						paymentResult.setResponseDesc("支付邮箱黑名单");
						return false;
					}
		...
		blackWhiteListDtos =riskBlackListService.getBlackWhiteList("8-2");
		if (null != blackWhiteListDtos && !blackWhiteListDtos.isEmpty()) {
			for (BlackWhiteListDto dto : blackWhiteListDtos) {
				// 账单邮箱，全匹配
				if (dto.getPartType() == 1) {
					if (dto.getContent().equalsIgnoreCase(shippingAddress)) {
						paymentResult.setResponseCode(getMessageId());
		...
		blackWhiteListDtos =riskBlackListService.getBlackWhiteList("9-2");
		if (null != blackWhiteListDtos && !blackWhiteListDtos.isEmpty()) {
			for (BlackWhiteListDto dto : blackWhiteListDtos) {
				// 账单邮箱，全匹配
				if (dto.getPartType() == 1) {
					if (dto.getContent().equalsIgnoreCase(billAddress)) {
						paymentResult.setResponseCode(getMessageId());
		...
		// 判断卡号
		blackWhiteListDtos =riskBlackListService.getBlackWhiteList("1-2");
		if (null != blackWhiteListDtos && !blackWhiteListDtos.isEmpty()
			...
			for (BlackWhiteListDto dto : blackWhiteListDtos) {
				...
				if (dto.getPartType() == 1) {
					if (dto.getContent().equals(cardHolderNumber)) {
						paymentResult.setResponseCode(getMessageId());
						paymentResult.setResponseDesc("银行卡号黑名单");
						return false;
					}
					...
					if(rst){
						paymentResult.setResponseCode(getMessageId());
						paymentResult.setResponseDesc("银行卡号黑名单");
						logger.info("银行卡号黑名单,正则表达式,content: "+dto.getContent());
						return false;
					}
				}
			}
		...
		blackWhiteListDtos =riskBlackListService.getBlackWhiteList("10-2");
		if (null != blackWhiteListDtos && !blackWhiteListDtos.isEmpty()
				 &&!StringUtil.isEmpty(cardHolderNumber)) {
			for (BlackWhiteListDto dto : blackWhiteListDtos) {
				if(dto.getPartType() == 4){
					...
					if(cardNo>=cardNo1&&cardNo<=cardNo2){
						paymentResult.setResponseCode(getMessageId());
						paymentResult.setResponseDesc("银行卡号黑名单");
						logger.info("银行卡号黑名单,IP地址段：value1: "
