 ...
 * @since 2010-09-13
 */
public class WithDrawResManualReAuditController extends WithdrawBaseController {
	...
	public ModelAndView search(HttpServletRequest request, HttpServletResponse response) throws Exception {
		String nodeName = WithDrawConstants.WITHDRAW_RESULT_PROCESS_MANUAL_REAUDIT_NODE;
	...
	public ModelAndView audit(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String reAuditBackReason = null;
			String auditStatus = request.getParameter("auditStatus");
			if (auditStatus.equals("0")) {
				reAuditBackReason = request.getParameter("reAuditBackReason");
			}

			String userId = SessionUserHolderUtil.getLoginId();
			//提交工作流的格式暂时是自定的,以nodename##1(同意)或者nodename##0(退回) manualFirstAudit
			...
			if (wkKeys != null) {
				...
				for (int i = 0; i < wkKeys.length; i++) {
					...
					taskMessage = WithDrawConstants.MANUAL_SECOND_AUDIT_NODE + "##" + auditStatus + "##";
					taskMessage += reAuditBackReason==null?"":reAuditBackReason;
					Map<String, Object> map = new HashMap<String, Object>();
					map.put("userId", userId);
					map.put("nodeName", WithDrawConstants.MANUAL_SECOND_AUDIT_NODE);
					...
					String resId = "";
					resId = this.wdOrdAuditService.withdrawResManualProcReAudit(workOrderId, reAuditBackReason, Integer
