...
public class AcctAllowInVoterTest extends AbstractTestNGSpringContextTests {
	@Resource(name = "fundout-securitycheck-AcctAllowInVoter")
	private Voter acctAllowInVoter;
	...
	public void test中间科目() {
		...
		// 收款方为中间科目，无需投票
		Assert.assertEquals(acctAllowInVoter.vote(principal, new DenyVoteMsg()), Voter.ABSTAIN);
	}
	...
	public void test正常会员() {
		...
		// 收款方为正常会员，应投票通过
		Assert.assertEquals(acctAllowInVoter.vote(principal, new DenyVoteMsg()), Voter.GRANTED);
	}
	...
	public void test止入会员() {
		...
		// 收款方为止入，应投票不通过
		Assert.assertEquals(acctAllowInVoter.vote(principal, new DenyVoteMsg()), Voter.DENIED);
	}
}
