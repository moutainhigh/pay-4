...
public class MonthLimitVoterTest extends AbstractTestNGSpringContextTests {
	@Resource(name = "fundout-securitycheck-MonthLimitVoter")
	private Voter monthLimitVoter;
	...
	public void test未超限() {
		...
		Assert.assertEquals(monthLimitVoter.vote(principal, new DenyVoteMsg()), Voter.GRANTED);
	}
	...
	public void test超限() {
		...
		Assert.assertEquals(monthLimitVoter.vote(principal, msg), Voter.DENIED);
		Assert.assertEquals(msg.getCode(), Constants.DENY_CODE_RISK_OUTOFAMT_MONTH);
	}
