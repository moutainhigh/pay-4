...
public class DayLimitVoterTest extends AbstractTestNGSpringContextTests {
	@Resource(name = "fundout-securitycheck-DayLimitVoter")
	private Voter dayLimitVoter;
	...
	public void test未超限() {
		...
		Assert.assertEquals(dayLimitVoter.vote(principal, new DenyVoteMsg()), Voter.GRANTED);
	}
	...
	public void test超限() {
		...
		Assert.assertEquals(dayLimitVoter.vote(principal, msg), Voter.DENIED);
		Assert.assertEquals(msg.getCode(), Constants.DENY_CODE_RISK_OUTOFAMT_DAY);
	}
