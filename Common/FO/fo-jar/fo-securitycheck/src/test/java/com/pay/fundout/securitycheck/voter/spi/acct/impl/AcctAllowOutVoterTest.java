...
@ContextConfiguration(locations = { "classpath*:context/spring/base/*.xml", "classpath*:config/env/test-dataAccessContext.xml", "classpath*:context/spring/**/*.xml",
		"classpath*:config/spring/**/*.xml", "classpath*:context/*.xml" })
public class AcctAllowOutVoterTest extends AbstractTestNGSpringContextTests {
	@Resource(name = "fundout-securitycheck-AcctAllowOutVoter")
	private Voter acctAllowOutVoter;
	...
	public void test正常会员() {
		...
		// 收款方为正常会员，应投票通过
		Assert.assertEquals(acctAllowOutVoter.vote(principal, new DenyVoteMsg()), Voter.GRANTED);
	}
	...
	public void test止出会员() {
		...
		// 收款方为止入，应投票不通过
		Assert.assertEquals(acctAllowOutVoter.vote(principal, new DenyVoteMsg()), Voter.DENIED);
	}
}
