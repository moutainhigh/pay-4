...
public class AcctAllowFundOutVoterTest extends AbstractTestNGSpringContextTests {
	@Resource(name = "fundout-securitycheck-AcctAllowFundOutVoter")
	private Voter acctAllowFundOutVoter;
	...
	public void test正常会员() {
		...
		// 收款方为正常会员，应投票通过
		Assert.assertEquals(acctAllowFundOutVoter.vote(principal, new DenyVoteMsg()), Voter.GRANTED);
	}
	...
	public void test止出会员() {
		...
		// 收款方为止入，应投票不通过
		Assert.assertEquals(acctAllowFundOutVoter.vote(principal, new DenyVoteMsg()), Voter.DENIED);
	}
