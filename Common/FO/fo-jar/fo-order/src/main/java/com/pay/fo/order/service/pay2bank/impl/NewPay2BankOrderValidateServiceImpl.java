 ...
 *
 */
public class NewPay2BankOrderValidateServiceImpl implements
	...
	public String validateRCLimitInfo(long memberCode,long paymentAmount,RCLimitResultDTO rcLimitInfo) {
		String message = null;
		if(null==rcLimitInfo){
			message = OrderValidateCode.NOT_SUPPORT_BIZ.getCode();//"暂不支持该业务";
		}else{
			if(paymentAmount>rcLimitInfo.getSingleLimit()){//单笔限额
				message = OrderValidateCode.OVER_SIGN_PAYMENT_LIMIT.getCode()+"-"+AmountUtils.numberFormat(rcLimitInfo.getSingleLimit());//"单笔最多付款"+AmountUtils.numberFormat(rcLimitInfo.getSingleLimit())+"元";
			}
			if(StringUtil.isNull(message)){
				...
				if(monthLimitTimes<dayLimitTimes){//如果每月剩余次数小于每日剩余次数,则先校验每月次数
					if(monthTimes+1 > rcLimitInfo.getMonthTimes()){ //每月限次  每月最多付款${pay2BankMonthLimitTimes}次，您今天还可以付款${allowPaymentTimes}次
						...
						message = OrderValidateCode.OVER_MONTH_PAYMENT_TIMES.getCode()+"-"+rcLimitInfo.getMonthTimes()+"-"+monthLimitTimes;//"每月最多付款"+rcLimitInfo.getMonthTimes()+"次，您今天还可以付款"+monthLimitTimes+"次";
					}
				}else{
					if(dayTimes+1> rcLimitInfo.getDayTimes()){ //每日限次
						...
						message = OrderValidateCode.OVER_DAY_PAYMENT_TIMES.getCode()+"-"+rcLimitInfo.getDayTimes()+"-"+dayLimitTimes;//"每日最多付款"+rcLimitInfo.getDayTimes()+"次，您今天还可以付款"+dayLimitTimes+"次";
			...
			if(StringUtil.isNull(message)){
				...
				if(monthLimitAmount<dayLimitAmount){//如果每月剩余限额小于每日剩余限额,则先校验每月限额
					if(paymentAmount+currentMonthAmount > rcLimitInfo.getMonthLimit()){//每月限额
						...
						message = OrderValidateCode.OVER_MONTH_PAYMENT_LIMIT.getCode()+"-"+AmountUtils.numberFormat(rcLimitInfo.getMonthLimit())+"-"+AmountUtils.numberFormat(monthLimitAmount);//"每月最多付款"+AmountUtils.numberFormat(rcLimitInfo.getMonthLimit())+"元，您今天还可以付款"+AmountUtils.numberFormat(monthLimitAmount)+"元";
					}
				}else{
					if(paymentAmount+currentDayAmount>rcLimitInfo.getDayLimit()){//每日限额
						...
						message = OrderValidateCode.OVER_DAY_PAYMENT_LIMIT.getCode()+"-"+AmountUtils.numberFormat(rcLimitInfo.getDayLimit())+"-"+AmountUtils.numberFormat(dayLimitAmount);//"每日最多付款"+AmountUtils.numberFormat(rcLimitInfo.getDayLimit())+"元，您今天还可以付款"+AmountUtils.numberFormat(dayLimitAmount)+"元";
					}
				}
			}
		}
