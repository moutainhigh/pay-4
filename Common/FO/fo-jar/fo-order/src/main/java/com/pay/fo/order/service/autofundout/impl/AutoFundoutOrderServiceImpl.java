 ...
 * 
 */
package com.pay.fo.order.service.autofundout.impl;
 ...
 *
 */
public class AutoFundoutOrderServiceImpl implements AutoFundoutOrderService {
	...
	public void createOrder(FundoutOrderDTO order) {
		try {
			...
			order.setProcessType(OrderProcessType.TRUST_WITHDRAW_REQ);
			orderAfterProcessService.process(order, orderCallbackService, reqAccountingService);
		} catch (Exception e) {
			LogUtil.error(this.getClass(), "委托提现", OPSTATUS.EXCEPTION, "", "", e.getMessage(), "", "存储订单信息或更新余额发生异常");
	...
	public boolean updateOrderStatus(FundoutOrderDTO order, int oldStatus) {
		try {
			...
			LogUtil.error(this.getClass(), "委托提现", OPSTATUS.EXCEPTION, "", "", e.getMessage(), "", "更新订单状态发生异常");
			log.error(e);
			return false;
		}
	...
	public void foProcessSuccess(FundoutOrderDTO foOrder ) {
		...
		foOrder.setProcessType(OrderProcessType.TRUST_WITHDRAW_SUCCESS);
	...
	public void foProcessFail(FundoutOrderDTO foOrder ) {
		...
		foOrder.setUpdateDate(new Date());
		foOrder.setProcessType(OrderProcessType.TRUST_WITHDRAW_FAIL);
	...
	public void refundOrder(FundoutOrderDTO foOrder ) {
		...
		foOrder.setUpdateDate(new Date());
		foOrder.setProcessType(OrderProcessType.TRUST_WITHDRAW_REFUND);
