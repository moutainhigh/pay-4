 ...
 * 
 */
package com.pay.fo.order.service.enforcewithdraw.callback;
 ...
 *
 */
public class EnforceWithdrawOrderCallbackServiceImpl extends
	...
	public boolean updateOrderStatus(Order order) throws AppException {
		...
		if(dto.getProcessType().getValue().equalsIgnoreCase(OrderProcessType.FORCE_WITHDRAW_REQ.getValue())){
			fundoutOrderProcessService.updateOrderStatusRdTx(dto, OrderStatus.INIT.getValue());
		}else if(dto.getProcessType().getValue().equalsIgnoreCase(OrderProcessType.FORCE_WITHDRAW_REFUND.getValue())){
	...
	public void notify(Order order) {
		...
		if(dto.getProcessType().getValue().equalsIgnoreCase(OrderProcessType.FORCE_WITHDRAW_REQ.getValue())){
			...
			if(isEmail(dto.getPayerLoginName())){
				notifyPayerEmail(dto, payerEmailTemplateMap.get(dto.getOrderStatus()), "您已经申请提现,等待支付向银行账户打款");
			}else{
				notifyPayerSMS(dto, payerSmsTemplateMap.get(dto.getOrderStatus()),"提现申请提交成功");
			}
		}else if(dto.getProcessType().getValue().equalsIgnoreCase(OrderProcessType.FORCE_WITHDRAW_FAIL.getValue())){
			if(isEmail(dto.getPayerLoginName())){
				notifyPayerEmail(dto, payerEmailTemplateMap.get(dto.getOrderStatus()), "您的申请提现处理失败");
			}else{
				notifyPayerSMS(dto, payerSmsTemplateMap.get(dto.getOrderStatus()),"提现失败");
			}
		}else if(dto.getProcessType().getValue().equalsIgnoreCase(OrderProcessType.FORCE_WITHDRAW_SUCCESS.getValue())){
			if(isEmail(dto.getPayerLoginName())){
				notifyPayerEmail(dto, payerEmailTemplateMap.get(dto.getOrderStatus()), "您的申请提现处理成功");
			}else{
				notifyPayerSMS(dto, payerSmsTemplateMap.get(dto.getOrderStatus()),"提现成功");
			}
		}
