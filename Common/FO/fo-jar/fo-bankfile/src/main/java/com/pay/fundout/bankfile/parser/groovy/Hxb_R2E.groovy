 ...
 *  @Description
 */
class Hxb_R2E extends AbstractBaseFileParser {
	...
	protected void parserFile(FileParseResult fileParseResult) throws Exception {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			...
			String status = trimAllWhitespace(String.valueOf(row.getCell(10)));
			...
			if (BusinessSupport.isBankStatusSuccess(status)) {
				...
				tempInfo.setBankFailureReason(String.valueOf(row.getCell(11)));
			} else {
