 ...
 *  @author lIWEI
 *  @Date 2011-4-15
 *  @Description 工商银行文件解析器
 *  @Copyright Copyright (c) 2004-2013 pay.com . All rights reserved. 版权所有
 */
public class ICBCReviewFoFileServiceImpl extends AbstractReviewFoFileService {
	...
	protected List<ReviewFoFileDTO> parserImportFile(InputStream inputStream) {
		...
		try {
			...
			while (line != null && line.length() != 0) {
				...
				reviewFoFileDTO.setAmount(new BigDecimal(lineArray[6].replaceAll("元", "")));//金额
	...
	protected List<ReviewFoFileDTO> parserLoadLocalFile(InputStream inputStream) {
		...
		try {
			...
			while (line != null && line.length() != 0) {
				String[] lineArray = line.split("\\|");
				ReviewFoFileDTO reviewFoFileDTO = new ReviewFoFileDTO();
				reviewFoFileDTO.setUsage(lineArray[16]);//附言用途，含订单流水
				reviewFoFileDTO.setAmount(new BigDecimal(lineArray[15]).divide(new BigDecimal(1000)));//金额
				reviewFoFileDTO.setPayeeAccountNo(lineArray[13]);//收款人账号
				reviewFoFileDTO.setPayeeName(lineArray[14]);//收款人名称
				reviewFoFileDTOs.add(reviewFoFileDTO);
				line = buffered.readLine();
			}
		} catch (IOException e) {
