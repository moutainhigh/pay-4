 ...
 * @author Sandy_Yang
 */
public class WithdrawDayTimesConfigRule extends MessageRule {
	...
	protected boolean makeDecision(Object validateBean) throws Exception {
		...
		if (!withDrawRequest.isBusiness()) {
			rcLimitResultDTO = foRcLimitFacade.getUserRcLimit(RCLIMITCODE.FO_PERSONAL_WITHDRAW.getKey(), null, Long.valueOf(withDrawRequest.getMemberCode())); //个人用户
		}else {
		...
		int dayTimes = withdrawOrderService.dayTimesTotal(Long
				.valueOf(withDrawRequest.getMemberCode()));
		
		if (dayTimes >= rcLimitResultDTO.getDayTimes()) {
