package com.pay.poss.service.ordercenter.fundin.impl.recharge;
 ...
 * @see
 */
public class OrderCenterRechargeServiceImpl extends AbstractOrderCenterCommonService{

	private final static Integer ACC_TYPE_RMB = 10;
	private static Log logger = LogFactory.getLog(OrderCenterRechargeServiceImpl.class);
	private OrderCenterRechargeDao orderCenterRechargeDao;
	private PEService peService;
	public void setPeService(PEService peService) {
		this.peService = peService;
	}
	public void setOrderCenterRechargeDao(
			final OrderCenterRechargeDao param) {
		this.orderCenterRechargeDao = param;
	}
	...
	public Map<String, Page<OrderCenterResultDTO>> queryResultList(
		...
		map.put("page", orderCenterRechargeDao.queryRechargeResultList(page, dto));
		return map;
	}
	...
	public OrderDetailDTO queryOrderDetail(OrderCenterQueryDTO queryDto) {
		...
		try {
			...
			orderDetailDTO = orderCenterRechargeDao.queryOrderDetail(primaryKy);
...
//			orderDetailDTO.setOrderWebsite(String);  //交易网址
			...
			orderDetailDTO.setPayeeLevel((null!=payeeBO) ? String.valueOf(payeeBO.getServiceLevelCode()):"");  //收款人用户等级
...
//			orderDetailDTO.setPayerId((null!=payerAcct) ? payerAcct.getAttribId().toString(): "");  //付款人用户标识
...
//			orderDetailDTO.setRemarks(String);  //留言
	...
	public Map<String, Object> queryOrderRelation(OrderCenterQueryDTO queryDto) {
		...
		map.put("relationList", orderCenterRechargeDao.queryRefundRelationList(queryDto));
		return map;
	}
