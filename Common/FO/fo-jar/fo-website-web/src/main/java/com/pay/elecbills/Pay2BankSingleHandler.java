 ...
 * 单笔付款到银行
 */
public class Pay2BankSingleHandler extends BaseElecBillsService {
	...
	public ElecBillsDto getViewData(Map<String, Object> map){
		...
		elecBillsDto.setTradeNo(resultMap.get("sequenceId")+"");
		elecBillsDto.setPayerName(memberInfo.getMemberName());
		elecBillsDto.setPayeeName(resultMap.get("payeeName") + "");
		elecBillsDto.setPayerLoginName(loginSession.getLoginName());
		elecBillsDto.setPayeeBankNo(resultMap.get("payeeLoginName")+"");//收款方银行账号命名payeeLoginName
		elecBillsDto.setAgreementPayName(getAgreementPayName());//协议代付方用户名
		elecBillsDto.setBankBranch(resultMap.get("bankBranch") + ""); //收款方开户行
		
		elecBillsDto.setAgreementPayBankName(bankAddress);//协议代付方开户行
		elecBillsDto.setAgreementPayBankNo(bankNo);//协议代付方账号
		elecBillsDto.setAmount(resultMap.get("amount")+"");
