 ...
 *
 */
public class BatchPay2AcctAuditController extends
	...
	@OperatorPermission(operatPermission = "FO_MASSPAY2ACCOUNT_CHECK")
	...
	public ModelAndView list(HttpServletRequest request,
		...
		try{
			...
			auditQueryInfo.setRequestType(OrderType.BATCHPAY2ACCT.getValue());
	...
	public ModelAndView detail(HttpServletRequest request,
		...
		try{
			...
			setBatchPay2AcctToken(request, command, "audit");
			...
			try{
				...
				if(StringUtil.isNull(message)){
					if(!StringUtil.isNull(command.getAuditRemark())&&(command.getAuditRemark().contains("<")||command.getAuditRemark().contains("%3c"))){
						message =  "请输入有效的备注";
					}else if(!BatchPay2AcctValidateUtils.vertifyReason(StringUtil.null2String(command.getAuditRemark()))){
						message = "请输入有效的备注";
					}
				}
				...
				if(StringUtil.isNull(message)){
					dto = batchPaymentReqBaseInfoService.getBatchPaymentReqBaseInfo(memberCode, OrderType.BATCHPAY2ACCT.getValue(), command.getBusinessBatchNo());
				  if(dto==null){
					  message = "无效的请求";
				...
				if(!StringUtil.isNull(message)){
					...
					if(workOrderStatus==2){
						...
						message = batchPay2AcctOrderValidateService.validateRCLimitInfo(memberCode, dto.getValidAmount(), dto.getValidCount());
					    if(StringUtil.isNull(message)){
						...
						if(!StringUtil.isNull(message)){
						  ...
						  batchPay2AcctRequestService.auditPassRequestRdTx(command.getRequestSeq(), loginSession.getOperatorIdentity(), command.getAuditRemark());
					    }
					}else{
						batchPay2AcctRequestService.auditRejectRequestRdTx(command.getRequestSeq(), loginSession.getOperatorIdentity(), command.getAuditRemark());
					}
				}
	...
	private void setBatchPay2AcctToken(HttpServletRequest request,
		...
		request.getSession().setAttribute("audit_token_batchpay2acct", token);
	}
	...
	private static boolean validateToken(HttpServletRequest request){
		...
		token_batchpay2acct = StringUtil.null2String(request.getSession().getAttribute("audit_token_batchpay2acct"));
		request.getSession().removeAttribute("audit_token_batchpay2acct");
