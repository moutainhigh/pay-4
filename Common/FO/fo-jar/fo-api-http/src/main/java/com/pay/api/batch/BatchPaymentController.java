 ...
 * 
 */
public class BatchPaymentController extends MultiActionController {

	private Log logger = LogFactory.getLog(BatchPaymentController.class);
	...
	public ModelAndView index(HttpServletRequest request,
		...
		String resultXml = messageHandleService.process(command
				.getMerchantCode(), command.getBizNo(), xml);

		response.getWriter().print(
				new String(Base64Util.encode(resultXml.getBytes())));

		return null;
	}

	public void setMessageHandleService(
			IMessageHandleService messageHandleService) {
		this.messageHandleService = messageHandleService;
	}

	private MessageCommand transferMessageCommand(HttpServletRequest request,
			String serviceCode) {
