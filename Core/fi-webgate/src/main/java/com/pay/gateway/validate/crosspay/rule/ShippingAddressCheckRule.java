 ...
 * 验证网关版本
 */
public class ShippingAddressCheckRule extends MessageRule {
	...
	protected boolean makeDecision(Object validateBean) throws Exception {
		...
		String shippingAddress = crosspayApiRequest.getShippingAddress();
		String mcc = crosspayApiRequest.getMcc();
		
		if(TradeMccEnum.OBJ_TRADE.getCode().equals(mcc)){//电商类交易
			if (StringUtil.isEmpty(shippingAddress)||shippingAddress.trim().length()>128) {
	        ...
	        if (!StringUtil.isEmpty(shippingAddress)
					 &&shippingAddress.trim().length()>128) {
				crosspayApiResponse.setResultCode(getMessageId());
				crosspayApiResponse.setResultMsg(getMessage());
				return false;
			}
