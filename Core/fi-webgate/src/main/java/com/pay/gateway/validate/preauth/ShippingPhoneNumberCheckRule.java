 ...
 * 验证网关版本
 */
public class ShippingPhoneNumberCheckRule extends MessageRule {
	...
	protected boolean makeDecision(Object validateBean) throws Exception {
		...
		String shippingPhoneNumber = crosspayApiRequest
				.getShippingPhoneNumber();
		String mcc = crosspayApiRequest.getMcc();
		
		if(TradeMccEnum.OBJ_TRADE.getCode().equals(mcc)){//电商类交易
			if (StringUtil.isEmpty(shippingPhoneNumber)||shippingPhoneNumber.trim().length()>64) {
				crosspayApiResponse.setResultCode(getMessageId());
				crosspayApiResponse.setResultMsg(getMessage());
				return false;
			}
		}else{
	        if (!StringUtil.isEmpty(shippingPhoneNumber)
					 &&shippingPhoneNumber.trim().length()>64) {
				crosspayApiResponse.setResultCode(getMessageId());
				crosspayApiResponse.setResultMsg(getMessage());
				return false;
			}
