 ...
 * 验证网关版本
 */
public class ShippingCityCheckRule extends MessageRule {
	...
	protected boolean makeDecision(Object validateBean) throws Exception {
		...
		String shippingCity = crosspayApiRequest.getShippingCity();
		String mcc = crosspayApiRequest.getMcc();
		
		if(TradeMccEnum.OBJ_TRADE.getCode().equals(mcc)){//电商类交易
			if (StringUtil.isEmpty(shippingCity)||shippingCity.trim().length()>128) {
				crosspayApiResponse.setResultCode(getMessageId());
				crosspayApiResponse.setResultMsg(getMessage());
				return false;
			}
		}else{
	        if (!StringUtil.isEmpty(shippingCity)
					 &&shippingCity.trim().length()>128) {
				crosspayApiResponse.setResultCode(getMessageId());
				crosspayApiResponse.setResultMsg(getMessage());
				return false;
			}
