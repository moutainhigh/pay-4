...
public class CrosspayCashierAcquireController extends MultiActionController {
	...
	public ModelAndView index(HttpServletRequest request,
		...
		try {
			...
			String orderId = crosspayRequestDTO.getOrderId() ;
			unComplicateparam.put("key", orderId);
			unComplicateparam.put("value", crosspayRequestDTO.getPartnerId());
			//5秒内不得重复提交订单
			unComplicateparam.put("seconds", "5");
			boolean complicateflag =redisClient.unComplicate(unComplicateparam);
			if(complicateflag==false){
				gatewayResponseDTO.setResultCode("0049");
				gatewayResponseDTO.setResultMsg("订单号重复");
				if("en".equals(crosspayRequestDTO.getLanguage())){
					gatewayResponseDTO.setResultMsg("Repeated transaction number");
				}
			}else{
