 ...
 * 验证网关版本
 */
public class ShippingStateCheckRule extends MessageRule {
	...
	protected boolean makeDecision(Object validateBean) throws Exception {
		...
		String shippingState = crosspayApiRequest.getShippingState();

		String mcc = crosspayApiRequest.getMcc();
		
		if(TradeMccEnum.OBJ_TRADE.getCode().equals(mcc)){//电商类交易
			if (StringUtil.isEmpty(shippingState)||shippingState.trim().length()>128) {
				crosspayApiResponse.setResultCode(getMessageId());
				crosspayApiResponse.setResultMsg(getMessage());
				return false;
			}
		}else{
	        if (!StringUtil.isEmpty(shippingState)
					 &&shippingState.trim().length()>128) {
				crosspayApiResponse.setResultCode(getMessageId());
				crosspayApiResponse.setResultMsg(getMessage());
				return false;
			}
