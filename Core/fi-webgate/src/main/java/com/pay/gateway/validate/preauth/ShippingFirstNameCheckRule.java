 ...
 * 验证网关版本
 */
public class ShippingFirstNameCheckRule extends MessageRule {
	...
	protected boolean makeDecision(Object validateBean) throws Exception {
		...
		String shippingFirstName = crosspayApiRequest.getShippingFirstName();

		String mcc = crosspayApiRequest.getMcc();

		if(TradeMccEnum.OBJ_TRADE.getCode().equals(mcc)){//电商类交易
			if (StringUtil.isEmpty(shippingFirstName)||shippingFirstName.trim().length()>64) {
				crosspayApiResponse.setResultCode(getMessageId());
				crosspayApiResponse.setResultMsg(getMessage());
				return false;
			}
		}else{
	        if (!StringUtil.isEmpty(shippingFirstName)
					 &&shippingFirstName.trim().length()>64) {
				crosspayApiResponse.setResultCode(getMessageId());
				crosspayApiResponse.setResultMsg(getMessage());
				return false;
			}
