...
public class GatewayRequestSaveAction extends AbstractAction {
	...
	protected void doExecute(Object validateBean) throws Exception {
		...
		String requestIP = wechatAlipayRequestDTO.getCustomerIP();
		Long memberCode = Long.valueOf(wechatAlipayRequestDTO.getPartnerId());
		Long requestId = saveRequest(requestIP, wechatAlipayRequestDTO);
		...
		wechatAlipayResponseDTO.setSellerName(memberBaseInfoBO.getName());
	}

	private Long saveRequest(String requestIP,
			WechatAlipayRequest crosspayRequestDTO) {
