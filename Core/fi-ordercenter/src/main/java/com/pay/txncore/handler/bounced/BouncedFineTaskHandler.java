...
public class BouncedFineTaskHandler implements EventHandler {

	public final Log logger = LogFactory.getLog(BouncedFineTaskHandler.class);
	...
	public String handle(String dataMsg) throws HessianInvokeException {
		...
		try {
			...
			paraMap.put("beginCreateDate", beginCreateDate);//本月的第一天
			paraMap.put("endCreateDate", endCreateDate);//当前日期
			//Visa交易数据
			...
			//查询拒付欺诈报表
			List<Map<String, Object>> fineRuleAndRate = bouncedfineTaskService
			...
			for (Map<String, Object> map : fineRuleAndRate) {
				...
				Long bouncedFineNo=bouncedfineTaskService.queryBouncedFineOrderNo();
				...
				bouncedfineTaskService.insertBouncedFine(map); // 创建 拒付罚款表
				bouncedfineTaskService.updateFineRuleAndRate(map);
			}
			resultMap.put("responseCode",
	...
	private void insertBouncedRateListForCardOrg(List<BouncedFraudResultDTO> BouncedFrauds,
		...
		{
			bouncedfineTaskService.batchCreate(insertBouncedRateList);
		}
	}
	/**
	 * 计算上月日期
