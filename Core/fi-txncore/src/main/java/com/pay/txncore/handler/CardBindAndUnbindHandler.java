 ...
 * @author chma
 */
public class CardBindAndUnbindHandler implements EventHandler {
	...
	public String handle(String dataMsg) throws HessianInvokeException {
		...
		try {
			...
			if(TradeTypeEnum.CARD_BIND.getCode().equals(tradeType)){
				paymentResult = cardBindService.bind(paymentInfo);
			}else if(TradeTypeEnum.CARD_UNBIND.getCode().equals(tradeType)){
				paymentResult = cardBindService.unBind(paymentInfo);
			}
