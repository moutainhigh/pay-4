...
import com.pay.txncore.utils.Blowfish;
...
public class TradeOrderServiceImpl implements TradeOrderService {
	...
	@Override
	public boolean updateTradeDataRnTx(TradeDataDTO dto, Integer oldStatus) {
		TradeData data = new TradeData();
		TradeOrderDTO orderDto = dto.getTradeOrderDTO();
		data.setTradeOrderNo(orderDto.getTradeOrderNo());
		String dataJsonStr = JSonUtil.toJSonString(dto.getData());
		try{
			String enDataJsonStr = blowfish.encrypt(dataJsonStr, dataJsonStr.length());
			data.setInputData(enDataJsonStr);
			data.setInputLength(dataJsonStr.length());
		}catch (Exception e){
			e.printStackTrace();
		}
		...
		TradeOrder tradeOrder = BeanConvertUtil.convert(TradeOrder.class,
				dto);
		tradeOrder.setOldStatus(oldStatus);
		return tradeDataDAO.update(data) && tradeOrderDAO.update(tradeOrder);
	}
	@Override
	public boolean updateTradeDataRnTx(TradeDataDTO dto) {
		TradeData data = new TradeData();
		TradeOrderDTO orderDto = dto.getTradeOrderDTO();
		data.setTradeOrderNo(orderDto.getTradeOrderNo());
		String dataJsonStr = JSonUtil.toJSonString(dto.getData());
		...
		TradeOrder tradeOrder = BeanConvertUtil.convert(TradeOrder.class,
				dto.getTradeOrderDTO());
		return tradeDataDAO.update(data) && tradeOrderDAO.update(tradeOrder);
	}
	
	@Override
	public boolean updateTradeBase(TradeBase dto) {
		return tradeBaseDAO.update(dto);
	}
