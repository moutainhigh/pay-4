...
public class OrderCreditOrderQueryHandler implements EventHandler{
	
	private static Logger logger = LoggerFactory.getLogger(OrderCreditOrderQueryHandler.class);
	...
	public String handle(String dataMsg) throws HessianInvokeException {
		...
		try {
			...
			if(pageMap==null){
				List<OrderCreditDTO> list = (List<OrderCreditDTO>) this.orderCreditService.findByCriteria("findByCriteria_orderCredit", paraMap) ;
				resultMap.put("result", list) ;
			}else{
				Page page = MapUtil.map2Object(Page.class, pageMap);
				List<OrderCreditDTO> list = (List<OrderCreditDTO>) this.orderCreditService.findByCriteria("findByCriteria_orderCredit", paraMap, page) ;
				resultMap.put("result", list) ;
