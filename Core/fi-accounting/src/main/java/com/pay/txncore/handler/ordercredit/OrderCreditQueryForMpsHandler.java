 ...
 *
 */
public class OrderCreditQueryForMpsHandler implements EventHandler{
	
	private static Logger logger = LoggerFactory.getLogger(OrderCreditQueryForMpsHandler.class);
		...
		try {
			...
			if (null != list && !list.isEmpty()) {	
				...
				while(it.hasNext()){
					...
					paymentService.BuildOrderCredit2(partnerSettlementOrder2, partnerSettlementOrder.getCreditCurrencyCode(),partnerSettlementOrder.getDayRate(), false);
				    if(partnerSettlementOrder2.getOrderAmountCny()<20000L){
				        it.remove();
				        //页码的总页数减1
