...
public class AutomaticTradeService7  extends QuartzJobBean{
	...
	public void automaticTrade() {
			...
			String startflag =readProperties.get("startflag5");
			String picitotal =readProperties.get("picitotal");
			Map paraMap = new HashMap();
			String memberCode=readProperties.get("memberCode5");
			paraMap.put("memberCode", memberCode);
			paraMap.put("startflag", startflag);
			String batchName=readProperties.get("batch5");
			paraMap.put("batch", batchName);
			paraMap.put("pflag", "P5");
			paraMap.put("picitotal", picitotal);
			paraMap.put("cardOrg", readProperties.get("cardOrg5"));
			...
			//第一次启动时
			 if(redisClient.get(startflag).equals("1")){
				 ...
				 if(redisClient.exists("P5")){
					 Map redisMap = redisClient.hgetAll("P5");
					...
					try {
						...
						Double spand =(double) (nowdate11-date11);
							...
							if(jiange>25){
								logger.info("p5调用核心....."+batchName);
								 txcoresubmit(paraMap);
								logger.info("p5调用核心结束....."+batchName);
							}
