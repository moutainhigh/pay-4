...
public class AutomaticTradeService6  extends QuartzJobBean{
	...
	public void automaticTrade() {
			...
			String startflag =readProperties.get("startflag4");
			String picitotal =readProperties.get("picitotal");
			Map paraMap = new HashMap();
			String memberCode=readProperties.get("memberCode4");
			paraMap.put("memberCode", memberCode);
			paraMap.put("startflag", startflag);
			String batchName=readProperties.get("batch4");
			paraMap.put("batch", batchName);
			paraMap.put("pflag", "P4");
			paraMap.put("picitotal", picitotal);
			paraMap.put("cardOrg", readProperties.get("cardOrg4"));
			...
			//第一次启动时
			 if(redisClient.get(startflag).equals("1")){
				 ...
				 if(redisClient.exists("P4")){
					 Map redisMap = redisClient.hgetAll("P4");
					...
					try {
						...
						Double spand =(double) (nowdate11-date11);
							...
							if(jiange>25){
								logger.info("p4调用核心....."+batchName);
								 txcoresubmit(paraMap);
								logger.info("p4调用核心结束....."+batchName);
							}
