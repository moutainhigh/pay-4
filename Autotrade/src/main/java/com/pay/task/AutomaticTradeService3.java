...
public class AutomaticTradeService3  extends QuartzJobBean{
	...
	public void automaticTrade() {
			...
			String startflag =readProperties.get("startflag");
			String picitotal =readProperties.get("picitotal");
			Map<String,String> paraMap = new HashMap();
			String memberCode=readProperties.get("memberCode1");
			paraMap.put("memberCode", memberCode);
			paraMap.put("startflag", startflag);
			String batchName=readProperties.get("batch1");
			paraMap.put("batch", batchName);
			paraMap.put("pflag", "P1");
			paraMap.put("picitotal", picitotal);
			paraMap.put("cardOrg", readProperties.get("cardOrg1"));
			...
			//第一次启动时
			 if(redisClient.get(startflag).equals("1")){
				 ...
				 if(redisClient.exists("P1")){
					 Map redisMap = redisClient.hgetAll("P1");
					...
					try {
						...
						Double spand =(double) (nowdate11-date11);
							...
							if(jiange>25){
								logger.info("调用核心....."+batchName);
								 txcoresubmit(paraMap);	
								logger.info("调用核心结束....."+batchName);
							}
